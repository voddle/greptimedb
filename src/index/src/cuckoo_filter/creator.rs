...
mod tests {
    ...
    async fn test_cuckoo_filter_creator() {
        ...
        let mut cfs = Vec::new();
        let mut offset = 0;
        for loc in meta.cuckoo_filter_locs {
            let cf_bytes = &bytes[offset..offset + loc.size as usize];
            let cf = CuckooFilter::deserialize(cf_bytes).unwrap();
            cfs.push(cf);
            offset += loc.size as usize;
        }

        // Check the first segment
        let cf0 = &cfs[0];
        assert!(cf0.contains(&b"a".to_vec()));
        assert!(cf0.contains(&b"b".to_vec()));
        assert!(!cf0.contains(&b"c".to_vec()));
        assert!(!cf0.contains(&b"d".to_vec()));

        // Check the second segment
        let cf1 = &cfs[1];
        assert!(!cf1.contains(&b"a".to_vec()));
        assert!(!cf1.contains(&b"b".to_vec()));
        assert!(cf1.contains(&b"c".to_vec()));
        assert!(cf1.contains(&b"d".to_vec()));
        assert!(cf1.contains(&b"e".to_vec()));
        assert!(cf1.contains(&b"f".to_vec()));
    }
}
